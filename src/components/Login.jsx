...
function Login() {
  ...
  return (
    <>
      <div className="min-h-screen flex items-center justify-center bg-black pt-3 pb-8 px-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-2xl mx-auto p-8 rounded-lg shadow-2xl border border-gray-800 text-gray-200 relative z-10 bg-black/80">
          ...
          <div className="space-y-4">
            {/* Admin Login Option */}
            <div
              className={`p-6 rounded-lg cursor-pointer transition-all duration-300 border border-gray-700 relative overflow-hidden group ${
                selectedOption === "admin" ? "ring-2 ring-cyan-400" : ""
              }`}
              style={{
                background: "gray-800",
              }}
              onMouseEnter={(e) => {
                if (selectedOption !== "admin") {
                  e.currentTarget.style.borderColor = "gray-900";
              ...
              onClick={() => selectLoginType("admin")}
            >
              <div className="w-14 h-14 mx-auto mb-4 rounded-full flex items-center justify-center text-2xl bg-gradient-to-br from-cyan-500 to-cyan-400 shadow-lg">
                👑
              </div>
              <h3 className="text-lg font-semibold text-center mb-2 text-white group-hover:text-cyan-300 transition-colors duration-200">
                Login as Admin
              </h3>
              <p className="text-center text-gray-400 text-sm leading-relaxed">
                Access administrative dashboard with full system controls and
            ...
            {showAdminKey && (
              <form
                ...
                className="mt-4 p-4 bg-black border border-gray-700 rounded-lg shadow-lg flex flex-col items-center"
              >
                <label className="text-gray-300 mb-3 font-medium">
                ...
                <input
                  ...
                  value={adminKey}
                  onChange={(e) => setAdminKey(e.target.value)}
                  className="w-full px-4 py-2 rounded bg-zinc-900 text-white border border-gray-700 focus:outline-none focus:ring-2 focus:ring-cyan-400 mb-3 transition-all duration-200"
                  autoFocus
                />
                {adminKeyError && (
                  <div className="text-red-400 mb-3 text-sm">{adminKeyError}</div>
                )}
                <div className="flex gap-3 w-full">
                  <button
                    type="submit"
                    className="flex-1 bg-white hover:bg-gray-100 text-black px-4 py-2 rounded font-medium transition-all duration-200 transform hover:scale-105"
                  >
                    Submit
                  </button>
                  <button
                    type="button"
                    className="flex-1 bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded font-medium transition-all duration-200 border border-gray-600"
                    onClick={() => {
                      setShowAdminKey(false);
                      setAdminKey("");
                      setAdminKeyError("");
            ...
            <div className="flex items-center my-8">
              ...
              <span className="px-3 text-gray-500 font-medium text-xs tracking-wide">
                OR
              </span>
              <div className="flex-1 h-px bg-gradient-to-r from-transparent via-gray-700 to-transparent"></div>
            </div>
            ...
            <div
              className={`p-6 rounded-lg cursor-pointer transition-all duration-300 border border-gray-700 relative overflow-hidden group ${
                selectedOption === "user" ? "ring-2 ring-cyan-400" : ""
              }`}
              style={{
                background: "gray-800",
              }}
            ...
            >
              ...
              <p className="text-center text-gray-400 text-sm leading-relaxed">
                Access your personal account with standard user privileges and
                features
              </p>
            </div>
          </div>
